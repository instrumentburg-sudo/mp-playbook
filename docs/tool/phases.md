...
2. **Анти-фильтр**: проверяем, что не попадает в текущий ассортимент селлера — строительный инструмент, наборы инструмента, ручной инструмент, автоинструмент. Если попадает — предупреждение «возможна самоканнибализация» + ручное подтверждение.
...
Полный маппинг «правило → эндпоинт → параметры» + референс-скрипт дискавери: [mpstats-api-catalog](../updates/mpstats-api-catalog.md).
...
**Pre-check**: проверяем, что в Фонде новинок хватает на текущую закупку + буфер на следующую (≥3 средних закупки). Подробно — [budget-cycle](budget-cycle.md). Если фонд меньше — ждём накопления, не стартуем.
